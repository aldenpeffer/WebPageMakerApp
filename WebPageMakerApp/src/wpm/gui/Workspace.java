...
public class Workspace extends AppWorkspaceComponent {
    ...
    static final String CLASS_TAG_TOOLBAR = "tag_toolbar";
    ...
    //Arraylist of tags
    ArrayList<HTMLTagPrototype> tagsList;
	...
	tags = new HashMap();
        ...
        //Remove button, part of top toolbar but not 
        //the tagButtons
        initRemoveButton();
	...
	dataManager.reset();
        ...
        //Loads tag types into arraylist
        tagsList = dataManager.getTags();
	...
	// CSS FILE
        tagToolbar.getStyleClass().add(CLASS_TAG_TOOLBAR);	
	...
	try {
		...
		int row = 1;
                ...
                //Variable containing name of selected tag
                String selectedTagName = selectedTag.getTagName();

                //Disables all buttons that aren't eligible children, enables
                //all buttons that are
                for (Button b : tagButtons){                    
                    for(HTMLTagPrototype tag : tagsList){
                        if(tag.getTagName().equals(b.getText())){
                            if(tag.getLegalParents().contains(selectedTagName)){
                                b.setDisable(false);
                            }else{
                                b.setDisable(true);
                            }                        
                ...
                //Disables removeButton if illegal node selected,
                //enables otherwise
	    ...
	    fileManager.exportData(dataManager, TEMP_PAGE);
            ...
            fileManager.createFiles(dataManager);
    ...
    public void initRemoveButton(){
        String imagePath = "file:./images/RemoveElement.png";
